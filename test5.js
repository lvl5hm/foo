...
(function () {
  ...
  window.telemedWidget = {
    init: function (containerSelector, authData, path, params) {
      ...
      getMaxHeight = typeof params.getMaxHeight === 'function' ? params.getMaxHeight : defaultGetMaxHeight;
    ...
    nav: function (where) {
      if (!where) {
        console.warn('Не передан маршрут навигации в метод telemedWidget.nav');
      }
  ...
  function findFrameContainer(containerSelector) {
    ...
    if (!container) {
      throw new Error('Контейнер "' + container + '" не найден');
    }
  }

  function setupMessagesProcessing() {
    window.addEventListener('message', function(evt) {
        return processMessage(evt.data);
    });
  }
  ...
  function authenticateAndRun(params, path, authData) {
    ...
    if (authData) {
      promise = auth(authData).then(function(result) {
        if (result.success) {
          console.info('Результат авторизации через виджет: ' + JSON.stringify(result));
  ...
  function initFrame(params, path, token) {
    // путь к приложению передавать обязательно, иначе не примет токен
    var fullPath = origin + '/' + (theme ? theme + '/' : '') + 'patient/' + (path || '');
    // параметры приложения для передачи во фрейм
    var urlParams = {
    ...
    // склеиваем непустые параметры в одну строку
    var paramString = Object.keys(urlParams)
      .filter(function(key) {return urlParams[key] !== undefined})
    ...
    window['iFrameResize']({
      ...
      initCallback: function() {
        postHeight();
        window.addEventListener('resize', function(){return postHeight()});
      }
    }, iframe);
  }
  ...
  function auth(authData) {
    return fetch(origin + '/api/partner/' + partnerCode + '/v2/authClient', {
      ...
      .catch(function(err) {
        console.error('Ошибка при вызове авторизации');
        console.error(err);
      })
      ...
      .then(function(json) {
          if (json) {
            ...
            showText('Приложение недоступно');
            return {
              auth: true,
              success: false
            };
  ...
  function postHeight() {
    ...
    maxHeight = typeof maxHeight === 'number' && maxHeight > 0 ? maxHeight : 800;
    sendMessage('iframe-parent-resize', maxHeight);
  }

  function sendMessage(type, value) {
    if (iframe && origin) {
      iframe.contentWindow.postMessage({type: type, value: value}, origin);
    } else {
      console.log('Система отправки сообщений во фрейм не инициализирована. Отправка невозможна');
    }
  }

  function getScriptOrigin() {
    var script = document.currentScript ? document.currentScript : document.querySelector('script[src$="' + theme + '/widget.js' + '"]');
    var origin = 'https://test-telemed.drclinics.ru';
